...
class CinderProxy(manager.SchedulerDependentManager):
    ...
    def _get_cascaded_image_service(self,context,cascaded_glance_url):
        ...
        #glance_api_servers = "https://162.3.128.80" + ":" + str(CONF.cascaded_glance_port)
    ...
    def _update_volumes(self, context, volumes):
        LOG.info("volumes_mapping_cache:%s"%self.volumes_mapping_cache['volumes'])
        LOG.info("cascaded_mapping_cache:%s"%self.cascaded_mapping_cache['volumes'])
        for volume in volumes:
            ...
            try:
                ...
                if ccding_vol_metadata.has_key('cascaed_updated_at') == False:
                    ...
                    cascading_updated_at = timeutils.isotime(timeutils.parse_strtime(ccding_vol_metadata['cascaed_updated_at']))
    ...
    def _update_cascading_image_status(self,context,image_service,image_id,disk_format):
        with image_utils.temporary_file() as tmp:
            with image_utils.temporary_file() as tmp_after_convert:
                LOG.debug("%s was raw, converting to %s" %
                          (image_id, disk_format))
                image_utils.convert_image(tmp, tmp_after_convert, disk_format)
        ...
        try:
            ...
            if cascaded_glance_url:
                ...
                def _wait_for_cascaded_image_active():
                    """Called at an interval until the cascaded image active."""
                    image_info = cascaded_image_service.show(context,  resp[1]['os-volume_upload_image']['image_id'])
                    if image_info['status'] == 'active':  
                        LOG.info(_("cascased volume copy to image successfully."))
