    ...
    def external_gateway_added(self, ri, ex_gw_port):
        ...
        if external_net_id:
            ...
            if subnet_ret and subnet_ret['subnets'] and len(subnet_ret['subnets']):
                ...
                #return
                cascaded_net = self.get_or_create_cascaded_net(external_net_id)
                cascaded_ext_net_id = cascaded_net['id']
                if not cascaded_ext_net_id:
                    LOG.error(_('ERR: can not get cascaded net_id from id'
                                ' %s by get_or_create_cascaded_net_id!'), external_net_id)
                    return

                external_subnet_id = ex_gw_port['subnet']['id']
                cascaded_ext_subnet_id = self.get_or_create_cascaded_subnet(external_subnet_id)['id']
                if not cascaded_ext_subnet_id:
                    LOG.error(_('ERR: can not get cascaded subnet_id from id'
                                ' %s by get_or_create_cascaded_net_id!'), external_subnet_id)
                    return
