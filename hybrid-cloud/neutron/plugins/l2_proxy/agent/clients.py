...
def get_cascade_neutron_client(mode):
    ...
    kwargs = {
                ...
                'auth_url': neutron_admin_auth_url,
                'insecure': cfg.CONF.AGENT.auth_insecure,
                'tenant_name': cfg.CONF.AGENT.neutron_admin_tenant_name
            }
    token = openStackClients.auth_token
    keystoneclient = kc.Client(**kwargs)
    token_info = keystoneclient.tokens._get(neutron_admin_auth_url+ "/tokens/%s" % token, 'access')
    tenant_id = token_info.tenant['id']
    return neutronClient, tenant_id
    ...
    def decorated_function(self, method_name, *args, **kwargs):
        ...
        while(True):
            try:
                ...
                retry = retry + 1
                if(retry <= 3):
                    self.client, self.tenant_id = get_cascade_neutron_client(self.mode)
                    continue
                else:
