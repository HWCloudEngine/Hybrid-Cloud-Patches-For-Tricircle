...
class OVSNeutronAgent(n_rpc.RpcCallback,
    ...
    def __init__(self, host, conf=None):
        ...
        self.cascading_tenant_id = self.cascading_neutron_client.get_tenant_id()
        self.cascaded_tenant_id = self.cascaded_neutron_client.get_tenant_id()
    ...
    def port_update_for_cascaded(self, port):
        ...
        elif port['device_owner'] == const.DEVICE_OWNER_ROUTER_INTF:
            req_props = {'name': 'router_port@' + port['id']}
        else:
    ...
    def _create_port(self, context, network_id, binding_profile, port_name,
        ...
        req_props = {'network_id': network_id,
                     ...
                     'device_owner': '',
                     'security_groups': [],
                     'tenant_id': self.cascaded_tenant_id
                     }
        try:
    ...
    def get_cascading_ports(self, context, filters):
        port_ret = self.cascading_neutron_client('list_ports', **filters)
        if port_ret and len(port_ret.get('ports')) > 0:
            return port_ret.get('ports')
        return
            ...
            if len(agent_ports):
                for agent_ip, ports in agent_ports.items():
                    ...
                    for mac_address, ips in mac_ip_map.items():
                        ...
                        req_props = {'mac_address': mac_address}
                        ports = self.get_cascading_ports(context, req_props)
                        if ports and len(ports) > 0:
                            port_id = ports[0]['id']
                        else:
                            continue
                        
                        port_name = 'remote_port@%s' % port_id
    ...
    def treat_devices_added_or_updated(self, devices, ovs_restarted):
        ...
        for details in devices_details_list:
            ...
            if 'port_id' in details:
                ...
                if(('compute' in details['device_owner']) or \
                    (details['device_owner'] == const.DEVICE_OWNER_ROUTER_INTF)):
        ...
        for pp,update in self._queue.each_update_to_next_port():
            ...
            if port_remove:
                ...
                if updated_port_info['binding:host_id'] == self.conf.host and \
                        (const.DEVICE_OWNER_COMPUTER in updated_port_info['device_owner'] or
                        updated_port_info['device_owner'] == const.DEVICE_OWNER_ROUTER_INTF):
