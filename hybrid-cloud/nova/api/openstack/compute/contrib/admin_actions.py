...
class AdminActionsController(wsgi.Controller):
    ...
    def _migrate(self, req, id, body):
        ...
        # add by liuling
        nic_info = param_dict.get('nics')
       
        security_groups = param_dict.get('security_groups')
        user_data = param_dict.get('user_data')
...
#             return volume_type_dist.get(availability_zone, None)
    ...
    def _delete_tmp_image(self,image_uuid,volume_dist_for_image_id): 
        ...
        if volume_dist_for_image_id:
            for image_id in volume_dist_for_image_id.values():
                ...
                cascaded_glance_url = self._get_cascaded_glance_url(self.availability_zone) or \
                                    self._get_cascaded_glance_url(self.source_availability_zone) 
                if cascaded_glance_url:
                     
                    url = urlparse.urlparse(cascaded_glance_url)
                    netloc = url.netloc
                    client = glance.GlanceClientWrapper(
                            ...
                            use_ssl=True,
                            version="1")
                    image_service = glance.GlanceImageService(client)
    ...
    def _get_cascaded_glance_url(self,region): 
        ...
        return self._get_management_url(keystoneclient, region,service_type='image')
     
    def _get_management_url(self, kc,region, **kwargs):
    ...
    def _upload_data_volume_to_image(self,data_volume_info_dict):
        ...
        for volume_id in data_volume_info_dict.keys():
            ...
            image_uuid_of_volume = response[1]['os-volume_upload_image']['image_id']
            volume_dist_for_image_id[volume_id] = image_uuid_of_volume
            cascaded_glance_url = self._get_cascaded_glance_url(self.availability_zone)
            if cascaded_glance_url:
                LOG.debug("begin create the cascaded image")
                image_meta={'container_format': 'bare', 'disk_format': 'qcow2',
                            'name':response[1]['os-volume_upload_image']['image_name'],
                            'id':response[1]['os-volume_upload_image']['image_id'],
                            'is_public':True}
                url = urlparse.urlparse(cascaded_glance_url)
                netloc = url.netloc
                client = glance.GlanceClientWrapper(
                ...
                with image_utils.temporary_file() as tmp:
                    with image_utils.temporary_file() as tmp_after_convert:
                        
                        image_utils.convert_image(tmp, tmp_after_convert, 'qcow2',
                                      run_as_root=True)
                
                       
                        with fileutils.file_open(tmp_after_convert, 'rb') as image_file:
                            image_service.create(self.context, image_meta,image_file)
                LOG.debug("end create the cascaded image")
            return volume_dist_for_image_id
    ...
    def _get_requested_networks(self, requested_networks):
        ...
        for network in requested_networks:
            ...
            try:
                try:
                    request.port_id = network.get('port-id', None) or network.get('port', None)
                except ValueError:
                ...
                if request.port_id:
                    ...
                    request.network_id = network.get('net-id',None) or network.get('uuid', None)
                ...
                try:
                    request.address =network.get('fixed_ip', None) or network.get('v4-fixed-ip', None) or network.get('v6-fixed-ip', None) or None
                except ValueError:
    ...
    def _create_instance(self,inst_type, boot_image_uuid,display_name=None, display_description=None,           
        ...
        while True:
            ...
            try:
                ...
                LOG.error('begin create instance %s' %(display_name))
...
#             sg_names = [sg['name'] for sg in self.security_groups
#                         if sg.get('name')]
#             security_groups = sg_names
        ...
        if self.nic_info is None:
            ...
            if not isinstance(self.nic_info, list):
                expl = _('Bad networks format')
                raise exc.HTTPBadRequest(explanation=expl)
            requested_networks =self.nic_info
        ...
        if instances is not None and len(instances) == 1:
            ...
            while instance_new.vm_state != 'active':
                ...
                try:
                    ...
                    LOG.error("get after migrate vm failed %s" %instance_new.uuid)
                    continue
                        
                query_new_vm_status_count =query_new_vm_status_count-1
                if instance_new.vm_state == 'error':
                   
                    if retry_create_vm !=0: 
                        LOG.error("bulid instance %s failed" %name)
                        retry_create_vm = retry_create_vm -1
                        self.compute_api.delete(self.context,instance_new)
                        instances=self._create_instance(inst_type,boot_image_uuid, display_name=name, 
                               display_description=name,key_name=key_name,metadata=metadata,
                               access_ip_v4=access_ip_v4,access_ip_v6=access_ip_v6,injected_files=injected_files,
                               admin_password=None, min_count=min_count,max_count=max_count, requested_networks=requested_networks,
                        ...
                        msg = _("migrate vm failed.")
                        raise exc.HTTPBadRequest(explanation=msg)
                elif query_new_vm_status_count ==0 and instance_new.vm_state != 'active':
                    msg = _("migrate vm failed.")
                    raise exc.HTTPBadRequest(explanation=msg)  
        ...
        try:
            ...
            # handle the floatingIp
            #get the interface-port info
            reused_floatingip_list = []
            ports_after_migrate = self._get_instance_port_info(instance_new)
            if floatingIp_fixIp_map:
                if ports_after_migrate:
                    floatingip_count = len(floatingIp_fixIp_map)
                    for port in ports_after_migrate:
                        while floatingip_count == 0:
                            break
                        ip_address = port['fixed_ips'][0].get('ip_address')
                        if ip_address in floatingIp_fixIp_map.keys():
                            self.network_api.associate_floating_ip(self.context,instance_new,\
                                                                   floatingIp_fixIp_map.get(ip_address)\
                                                                   ['floatingips'][0]['floating_ip_address'],ip_address)
                            reused_floatingip_list.append(floatingIp_fixIp_map.get(ip_address))
                            floatingip_count = floatingip_count -1
                        else:
                            net_uuid = port.get('network_id')
                            search_opts={'network_id':net_uuid,'device_owner':'network:router_interface_distributed'}
                            router_interface_ports_result = self.network_api.list_ports(self.context,**search_opts)
                            router_interface_ports = router_interface_ports_result.get('ports')
                            if router_interface_ports is None or len(router_interface_ports) ==0:
                                continue
                            router_interface_port = router_interface_ports[0]
                            router_id = router_interface_port.get('device_id')
                            if router_id:
                                router_info = self.network_api.show_router(self.context,router_id)
                                router = router_info.get('router')
                                external_gateway_info = router.get('external_gateway_info')
                                if external_gateway_info:
                                    ext_network = external_gateway_info.get('network_id')
                                    floatingip = self.network_api.allocate_floating_ip(self.context, ext_network) 
                                    self.network_api.associate_floating_ip(self.context,instance_new,  
...
#             
            #delete floatingip
            for floatingip in floatingIp_fixIp_map.values():
                if floatingip in  reused_floatingip_list:
                    continue
                else:
                    floatingip_uuid = floatingip.get('floatingips')[0]['id']
                    try:
                        floating_ip_ret = self.network_api.delete_floatingip(self.context,floatingip_uuid)
                        LOG.debug(_('delete floatingip for %s, Response:%s') % 
                                  (floatingip_uuid, str(floating_ip_ret)))
                    except Exception as e:
                        LOG.warning(_("delete floatingip for %s, failed: %s"), floatingip_uuid, e)
